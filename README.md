...
**Acesse a versão interativa em português: [Collision Detection PT-BR](https://araujosemacento.github.io/collision_detection/)**
...
- **Autor Original**: [Jeff Thompson](http://www.jeffreythompson.org/)
- **Livro Original em Inglês**: [Collision Detection](http://www.jeffreythompson.org/collision-detection/)
...
2. **Portabilidade dos Demonstrativos Gráficos**: Adaptando da forma mais fiel que consegui os 19 exemplos em Java/PDE para **p5.js**.
3. **Tradução e Preservação de Conteúdo**: Tradução completa ou parcial dos capítulos para o português do Brasil (PT-BR).
4. **Exemplos em Pygame (Python)**: Criação de implementações equivalentes em **Pygame** para quem desenvolve jogos em Python.
5. **Interface e UX**: Auxílio no desenvolvimento da interface do site, incluindo sistema de abas de código multilinguagem com persistência global em `localStorage` e suporte a temas Light e Dark Mode.
...
```bash
# Clone o repositório
git clone https://github.com/araujosemacento/collision_detection.git
